...
export function RestTimerBar({ remaining, onStart, onStop }: RestTimerBarProps) {
  ...
  return (
    <div className="sticky top-0 z-30 border-b border-white/[0.06] bg-[#111]/95 backdrop-blur-md">
      <div className="mx-auto flex max-w-[1400px] flex-wrap items-center justify-between gap-2 px-3 py-2.5 sm:px-4">
            ...
            >
              {([60, 90, 120] as const).map((s) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.96 }}
                  onClick={() => onStart(s)}
                  className="rounded-full border border-white/[0.1] bg-[#1e1e1e] px-3 py-1.5 text-xs font-semibold text-white/85 hover:bg-[#262626]"
                >
                  {s}s
                </motion.button>
              ))}
